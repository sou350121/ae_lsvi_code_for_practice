...
ORDERING = None
ORDERING = ['ae-lsvi', 'mts', 'mei', 'ei', 'ts', 'rand', 'revi']
...
def run():
    ...
    if len(ids) > 1 or sum(should_prop) > 0:
        ...
        fig, axs = plt.subplots(nrows=1, ncols=num_to_plot, figsize=(34, 10))
        plt.subplots_adjust(wspace=0.3, bottom=0.25, left=0.05, right=1)
    else:
    ...
    plot_idx = 0
    cmap = None
    cmap_labs = ['ae-lsvi', 'ei', 'ts', 'revi', 'mei', 'mts', 'rand']
    ...
    for run_idx, run_id in enumerate(ids):
        ...
        for res in loaded_results:
            ...
            for k, v in res.iteritems():
                if k == 'agn-ei' or k == 'ja-ei':
                    ...
                    new_res['mts'] = v
                elif k == 'aelsvi' or k == 'joint-aelsvi':
                    new_res['ae-lsvi'] = v
        ...
        if plot_opts.only_props:
            ...
            plot_total(f_infos, results, ax=axs[plot_idx], add_legend=add_legend,
                       ...
                       max_regret=True, add_ylabel=plot_idx==0)
        if titles is not None:
            axs[plot_idx].set_title(titles[plot_idx].replace('_', ' '), fontsize=48)
        plot_idx += 1
        add_legend = (not plot_opts.legend_on_first or plot_idx == 0) and plot_opts.use_legend
    ...
    if plot_opts.write_path is not None:
        plt.savefig(plot_opts.write_path, format='pdf')
